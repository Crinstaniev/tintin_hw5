import { tintin_hw5, idlFactory } from '../../declarations/tintin_hw5';
...
let load_followed = async () => {
  let followed = await tintin_hw5.follows();
...
let post = async () => {
  ...
  try {
    await tintin_hw5.post(otp, text);
  } catch (error) {
...
let load_posts = async () => {
  ...
  let posts = await tintin_hw5.posts(0);
  let name = await tintin_hw5.get_name();
